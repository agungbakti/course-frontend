...
function studentPortal(studentId) {
    ...
    const credit = getCredits(student.gpa)
    const matkul = getSubjects(credit)
...
function getSubjects(credits) {
    ...
    const wajib = subjectsList.filter(s => s.status === "wajib")
    const pilihan = subjectsList.filter(s => s.status === "pilihan").sort((a, b) => b.credit - a.credit)
    
    const gabungan = [...wajib, ...pilihan]

    let result = []

    let totalcredits = 0
    gabungan.forEach(matkul => {
        let jumlahCredit = matkul.credit
        if (totalcredits < credits) {
            totalcredits += jumlahCredit
            let object = {
                subjectName: matkul.subjectName,
                credit: matkul.credit,
...
// console.log(getCredits(3));
// console.log(getSubjects(12));
